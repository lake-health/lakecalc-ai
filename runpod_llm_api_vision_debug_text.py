...
def call_llama_text(text_content):
    """Call Llama 7B for text-based PDFs with debug output"""
    
    print(f"DEBUG: Text content being sent to Llama: {text_content[:200]}...")
...
async def parse_document(file: UploadFile = File(...)):
    try:
        ...
        if file.filename.lower().endswith('.txt'):
            print("Text file detected, using Llama 7B")
            with open(file_path, 'r', encoding='utf-8') as f:
                text_content = f.read()
            
            print(f"DEBUG: Read text content: {text_content[:200]}...")
            result = call_llama_text(text_content)
        else:
            return {"error": "Only text files supported in this debug version"}
...
@app.get("/")
async def root():
    return {"message": "LakeCalc AI Parser API - Text Debug Version"}
