...
def call_llava_vision_pdf(image_path):
    ...
    # Simple debug prompt - just ask what it sees
    prompt = """Look at this medical document image and tell me:

1. What language is this document in?
2. Can you see any patient name or personal information?
3. Can you see any age or birth date information?
4. What type of medical document is this?

Please describe what you see in detail, then extract the biometry data in JSON format:

{
  "patient_name": "extract actual name or null",
  "age": "extract actual age or null", 
...
{{
  ...
  "os": {{"axial_length": 23.7, "k1": 41.8, "k2": 43.2, "k_axis_1": 85, "k_axis_2": 175, "acd": 2.9, "lt": 4.6, "wtw": 11.2, "cct": 545}}
    ...
    try:
        response = requests.post("http://127.0.0.1:5001/generate", json=payload, timeout=60)
        response.raise_for_status()
        return response.json()
    except Exception as e:
...
@app.get("/")
async def root():
    return {"message": "LakeCalc AI Parser API - Debug Version"}
