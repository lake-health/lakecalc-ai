...
# v3.8 + optional LLM fallback (OpenAI) for last-mile rescue when key fields remain missing
...
try:
    ...
    # Library not available or not configured; fallback remains disabled
    OPENAI_ENABLED = False
...
def smart_fix_cct_bound(text_with_bindings: str) -> str:
    ...
    def replace(match: re.Match) -> str:
        ...
        if mf:
            cand = mf.group(1)
            try:
                if plausible_um(cand):
                    return full.replace(val, cand)
            except Exception:
                pass
        start = match.start()
        bgn   = max(0, start - 200)
    ...
    try:
        for page_layout in extract_pages(BytesIO(pdf_bytes), laparams=laparams):
            ...
            def harvest_column(col_lines: List[tuple]) -> Dict[str,str]:
                ...
                label_map = {
                    "AL":"axial_length","ACD":"acd","LT":"lt","WTW":"wtw","CCT":"cct",
                    "K1":"k1","K2":"k2","AK":"ak","ΔK":"ak","K":"ak"
                }
                label_rows = []
...
# =========================
def llm_enrich(raw_text: str, current: dict) -> dict:
    """
    Use OpenAI only to fill clearly-missing fields. Keeps existing values.
    Expects environment OPENAI_API_KEY and openai==1.x installed.
    """
    if not OPENAI_ENABLED:
        return current

    # Determine which fields are still empty
    want_keys = [k for k in FIELD_ORDER if k != "source" and not (current.get(k) or "").strip()]
    if not want_keys:
        return current
    ...
    prompt = f"""
You are a careful data extractor for ophthalmology measurement sheets (IOL biometry, keratometry).
From the text below, extract ONLY the missing fields listed, preserving units and typical formatting
(e.g., "23.45 mm", "43.50 D @ 90°", "545 µm"). If a field truly isn't present, leave it empty.

Missing fields: {', '.join(want_keys)}
Return STRICT JSON only, matching this schema keys: {list(schema['properties'].keys())}.
Text:
<<<
{raw_text}
>>>
"""
    ...
    try:
        resp = openai_client.chat.completions.create(
            model=os.environ.get("LAKECALC_OPENAI_MODEL", "gpt-4o-mini"),
            messages=[
                {"role": "system", "content": "Extract only the requested ophthalmic scalar values with units; return strict JSON."},
        ...
        jtxt = content
        # Lenient bracket find
        m = re.search(r"\{.*\}", jtxt, flags=re.DOTALL)
        if m:
            jtxt = m.group(0)
        obj = json.loads(jtxt)
...
def parse_iol(norm_text: str, pdf_bytes: Optional[bytes], source_label: str, want_debug: bool = False):
    ...
    if left_txt is not None and right_txt is not None:
        ...
        # Optional LLM enrich per eye if still missing criticals
        def needs_llm(d: dict) -> bool:
            # prefer to trigger if CCT is missing or <= 3 total empties
            empties = [k for k in FIELD_ORDER if k != "source" and not (d.get(k) or "").strip()]
            return bool(empties)

        if OPENAI_ENABLED:
            for eye in ("OD", "OS"):
                if needs_llm(mapping[eye]):
                    mapping[eye] = llm_enrich(
                        (left_norm if (eye=="OD" and mapping is not None and debug["mapping"].startswith("OD<-left"))
                         else right_norm) if left_norm and right_norm else (left_norm or right_norm or norm_text),
                        mapping[eye]
                    )
    ...
    # LLM enrich single block (OD) if enabled
    if OPENAI_ENABLED:
        result["OD"] = llm_enrich(single, result["OD"])
