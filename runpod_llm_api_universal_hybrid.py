...
def extract_patient_info_with_ocr(image_path):
    """Extract patient name and age using OCR with comprehensive patterns"""
    try:
        # Use pytesseract to extract text
        text = pytesseract.image_to_string(Image.open(image_path))
        print(f"DEBUG: OCR extracted text: {text[:300]}...")
        
        # Extract patient name using comprehensive patterns
        patient_name = None
        name_patterns = [
            # English patterns
            r'PATIENT[:\s]+([A-Z\s,]+)',
            r'NAME[:\s]+([A-Z\s,]+)',
            r'PATIENT\s+NAME[:\s]+([A-Z\s,]+)',
            # Italian/Spanish patterns
            r'NOME[:\s]+([A-Z\s,]+)',
            r'PACIENTE[:\s]+([A-Z\s,]+)',
            r'PAZIENTE[:\s]+([A-Z\s,]+)',
            # French patterns
        ...
        # Extract age using comprehensive patterns
        age = None
        age_patterns = [
            # Direct age patterns
            r'AGE[:\s]+(\d+)',
            ...
            r'(\d{1,2})\s*ANOS?\s*DE\s*EDAD',
        ]
        
        for pattern in age_patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                if 'BIRTH' in pattern.upper() or 'NAISSANCE' in pattern.upper() or 'GEBURT' in pattern.upper() or 'NATO' in pattern.upper():
                    # Calculate age from birth year
                    birth_year = int(match.group(1))
                    age = 2024 - birth_year
                elif '/' in pattern or '-' in pattern:
                    # Date format - calculate age from birth date
                    if '/' in pattern:  # DD/MM/YYYY or MM/DD/YYYY
                        day, month, year = match.groups()
                        birth_year = int(year)
                        age = 2024 - birth_year
                    else:  # YYYY-MM-DD or DD-MM-YYYY
...
def call_llava_biometry_only(text_content):
    ...
    prompt = f"""Extract ONLY biometry measurements from this medical document text in JSON format:
...
@app.get("/")
async def root():
    return {"message": "LakeCalc AI Parser API - Universal Hybrid Version"}
