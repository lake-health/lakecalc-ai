#!/usr/bin/env python3
"""
RunPod LLM API with Vision Support and PDF-to-Image Conversion - Final Version with Aggressive Age Extraction
"""
import os
import json
import logging
...
async def call_llava_vision_pdf(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Create aggressive vision prompt for biometry parsing
        prompt = """You are a medical document parser. Extract biometry data from this medical document image.

CRITICAL INSTRUCTIONS - READ EVERYTHING CAREFULLY:
1. Look carefully at the ENTIRE document for patient information
2. Patient name is usually in the header, title, or patient information section
3. Age is often near patient name or in demographic information - look for numbers that could be age (typically 18-100)
4. Look for both OD (right eye) and OS (left eye) measurements
5. Extract axis values from patterns like "K1: 40.95 D @ 100°" where 100° is the axis

AGE EXTRACTION IS CRITICAL:
- Look for age in years (e.g., "65 years", "Age: 45", "45 anos", "Idade: 50", "65y", "65 y", etc.)
- Look for birth dates and calculate age if needed
- Look for demographic information sections
- Age is usually a number between 18-100
- If you see "65 anos" or "65 years" or "Age: 65", extract 65

PATIENT NAME EXTRACTION:
- Look for names in headers, titles, or patient data sections
- Extract the full name as it appears in the document
- Look for patterns like "Patient:", "Nome:", "Name:", etc.

Return ONLY valid JSON with:
- patient_name (string) - Extract the ACTUAL patient name from the document
- age (number) - Extract the ACTUAL age from the document  
- od (object with: axial_length, k1, k2, k_axis_1, k_axis_2, acd, lt, wtw, cct)
- os (object with: axial_length, k1, k2, k_axis_1, k_axis_2, acd, lt, wtw, cct)

IMPORTANT: 
- Do NOT use example names like "John Doe" - extract the REAL patient name
- Do NOT use example ages - extract the REAL age
- If you cannot find patient name or age, use null
- Look for patient information in headers, titles, or patient data sections

Example format (use ACTUAL data from the document):
{
  "patient_name": "[ACTUAL PATIENT NAME FROM DOCUMENT]",
