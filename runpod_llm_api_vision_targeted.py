#!/usr/bin/env python3
"""
RunPod LLM API with Vision Support - Targeted Version for Patient Info Extraction
"""
import os
import json
import logging
...
async def call_llava_vision_pdf(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Very targeted prompt for patient information extraction
        prompt = """Look at this medical document image and extract the biometry data.

CRITICAL: Look for patient information in these specific locations:
1. TOP of the document - headers, titles, patient info sections
2. Look for patterns like: "Patient:", "Name:", "Nome:", "Paciente:", "Patient Name:"
3. Look for age information near the patient name or in demographic sections
4. Look for birth dates and calculate age if needed

For biometry measurements, look for:
- OD (right eye) and OS (left eye) measurements
- Axial length, K1, K2 values with axis information
- ACD, LT, WTW, CCT measurements
...
- Look carefully at the top of the document for patient information"""
