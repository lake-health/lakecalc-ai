#!/usr/bin/env python3
"""
RunPod LLM API with Vision Support
Handles both text and image-based biometry parsing using LLaVA
"""
import os
import json
import logging
...
app = FastAPI(
    ...
    description="Vision-capable biometry parsing service with LLaVA",
    version="1.0.0"
)
...
async def parse_biometry(file: UploadFile = File(...)):
    """Parse biometry file using LLaVA vision model"""
    ...
    try:
        ...
        if file.filename.lower().endswith('.pdf'):
            # For PDFs, try text extraction first, then vision
...
async def call_llava_vision(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        prompt = """You are a medical biometry expert. Analyze this image and extract all biometry data in JSON format.

Look for:
- Patient information (name, birth date, age, ID)
- OD (Right Eye) data: AL (axial length), K1, K2, K1 axis, K2 axis, ACD, LT, WTW, CCT
- OS (Left Eye) data: AL (axial length), K1, K2, K1 axis, K2 axis, ACD, LT, WTW, CCT

Return ONLY valid JSON in this exact format:
{
  "patient_name": "string or null",
  "birth_date": "MM/DD/YYYY or null", 
  "age": number or null,
  "od": {
    "axial_length": number or null,
    "k1": number or null,
    "k2": number or null,
    "k_axis_1": number or null,
    "k_axis_2": number or null,
    "acd": number or null,
    "lt": number or null,
    "wtw": number or null,
    "cct": number or null
...
async def call_llama_7b(text: str) -> Dict[str, Any]:
    ...
    try:
        prompt = f"""You are a medical biometry expert. Extract all biometry data from this text in JSON format.
...
def calculate_confidence(data: Dict[str, Any]) -> float:
    ...
    # Check OD data
    od_data = data.get("od", {})
    for field in ["axial_length", "k1", "k2", "acd"]:
